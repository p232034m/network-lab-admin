...
class L2Switch(RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        #self.logger.info("datapathID : %d", datapath.id)
    ...
    def send_lldp_packet(self, datapath_id, port_no, dl_addr):
        ...
        if datapath is None:
            ...
            return
        #self.logger.info("send lldp_packet")
    ...
    def _packet_in_handler(self, ev):
        ...
        try:
            ...
            self.logger.info("%d time",i)
            self.logger.info("DatapathID : %d", datapath.id)
            self.logger.info("Packet-In")
            self.logger.info("  src=%s", src)
            self.logger.info("  dst=%s", dst)
            self.logger.info("  old_peer=%s", old_peer)

            output_json(datapath.id,src_dpid, dst_dpid, src_port_no, dst_port_no) # add 
...
def output_json(dpid,src_dpid, dst_dpid, src_port_no, dst_port_no):
    if not dpid in dpids:
        dpids.append(dpid)

    if (src_dpid,dst_dpid) in [(link.get("src"),link.get("dst")) for link in links] :
        return
    
    else:
        tmp_dict = {}
        tmp_dict["src"] = src_dpid
        tmp_dict["dst"] = dst_dpid
        tmp_dict["src_p"] = src_port_no
        tmp_dict["dst_p"] = dst_port_no 
        links.append(tmp_dict)

        dpids.sort()
        links_sort = sorted(links, key = lambda x:x["src"])
        
        json_dict = {}
        json_dict["datapathIDs"] = dpids
        json_dict["links"] = links_sort
        
        file = open("/home/kuratam/python/ryu_custom/cloudyswitch/cloudyswitch/app/lldp_topology.json", "w")
